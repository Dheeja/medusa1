...
The Index Module is an experimental feature that's subject to change. It is not recommended for production use.

</Note>

## What is the Index Module?

The Index Module adds a method to [Query](../query/page.mdx) that allows you to filter data by linked modules.
...
The Index Module solves this problem by injesting data into a central data store on application start up. The data store has a relational structure that enables efficiently filtering the data injested from different modules (and their data stores). So, when you retrieve data with the Index Module, you're retrieving them from the injested data store, not the original data source.
...
### Injested Data Models

Currently, only the following data models are injested by the Index Module when you install it:
...
The index module only injests data when you start your Medusa server. So, to injest the [currently supported data models](#injested-data-models), start the Medusa application:

```bash npm2yarn
npm run dev
```

The injestion process may take long if you have a large set of products. You'll see the following messages in the logs:
...
- `filters`: An object with the filters to apply on the data model's properties, relations, and linked data models that are injested.

---

## How to Injest Custom Data Models

Aside from the [injest Medusa data models](#injested-data-models), you can also injest your own custom data models into the Index Module. You can do so by defining a link between your custom data model and another injested data model, and setting the `filterable` property in the link definition.
...
For example, assuming you have a Brand Module with a Brand data model (as explained in the [Customizations](../../../customization/custom-features/module/page.mdx)), you can injest it into the Index Module using the `filterable` property in its link definition to the Product data model:
...
The `filterable` property is an array of property names in the data model that can be filtered using the `index` method. When the `filterable` property is set, the Index Module will injest into its data store the custom data model.
