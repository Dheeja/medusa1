...
export const metadata = {
  title: `${pageNumber} Migrate from Medusa v1 to v2`,
}

# {metadata.title}

In this chapter, you'll learn how to migrate your Medusa project from Medusa v1 to v2.

## What to Expect in This Chapter

This chapter is designed to help developers who have created projects with Medusa v1 to migrate them to Medusa v2. This guide will cover:

- The steps to update your project from Medusa v1 to v2.
- The high-level changes in the concepts and APIs of concepts and tools between Medusa v1 and v2.

By following this chapter, you'll learn about the general changes you need to make in your project, with links to read more about each topic.

This chapter is also useful for developers who are already familiar with Medusa v1 and want to learn about the main differences from Medusa v2. However, it doesn't cover all the new and improved concepts in Medusa v2. Instead, it's highly recommended to ready the rest of this documentation to learn about them.

---

## Upgrade from Medusa v1 to v2
...
It's highly recommended to fully go through this chapter before you actually update your application, as some v1 features may have been removed or heavily changed in v2. By doing so, you'll formulate a clearer plan for your migration.
...
For other data types, you'll probably need to migrate them manually through custom scripts.

{/* TODO add section about supported plugins */}
...
Medusa v2 comes with changes and improvements to its CLI tool. So, it's recommended to update your `package.json` with the following scripts:
...
In Medusa v1, you configured your application in the `medusa-config.js` file. Medusa supports this file as `medusa-config.ts`, so make sure to rename it.
...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      <Table.Cell>
        Not supported. Use [projectConfig.databaseDriverOptions](../../configurations/medusa-config/page.mdx#databasedriveroptions) instead.
      </Table.Cell>
    </Table.Row>
    <Table.Row>
      <Table.Cell>
    ...
    <Table.Row>
      ...
      <Table.Cell>
        Now an array of modules. [Learn more](../../configurations/medusa-config/page.mdx#module-configurations-modules)
      </Table.Cell>
    </Table.Row>
  </Table.Body>
</Table>
...
While the `plugins` configuration hasn't changed, plugins available in Medusa v1 are not compatible with Medusa v2.

{/* TODO add link */}

Refer to the Plugins section to learn more about plugin-related changes.
...
Refer to the Medusa Admin section to learn more about the changes in the admin dashboard.
...
The first command will create the database and tables for your v2 project. The second command will seed the database with demo data.

After that, you can start your Medusa application with the `dev` command. Note that you may have errors if you need to make implementation changes that are covered in the rest of this guide, so it's better to wait until you finish the v2 migration process before running starting the Medusa application.
...
The Medusa Admin is now available out-of-the-box. It's built with [Vite](https://vite.dev/) and runs at `http://localhost:9000/app` by default when you start your Medusa application.

### Admin Configurations

You previously configured the admin dashboard when you added it as a plugin in Medusa v1.

In Medusa v2, you configure the Medusa Admin in `medusa-config.ts`. The `defineConfig` utility accepts an `admin` property to configure the Medusa Admin.
...
Medusa v2 supports custom widgets similar to Medusa v1 with minor changes.

#### Widget Injection Zones

Due to design changes in the Medusa Admin, some widgets may have been added or removed. Refer to the [Admin Widgets Injection Zones](!resources!/admin-widget-injection-zones) reference for the full list of injection zones in v2.

#### Widget Configurations

In Medusa v1, you exported in the widget's file a `config` object with the widget's configurations, such as its injection zone.
...
Medusa v2 also supports custom UI routes similar to Medusa v1 with minor changes.

#### UI Routes Path Prefix
...
In Medusa v2, the `/a` prefix has been removed. So, the same route would be available at `http://localhost:9000/app/custom`.

#### Route Configuration Object

In v1, you exported a `config` object with the route's configurations to show the route in the dashboard's sidebar.
...
In Medusa v1, you create pages to be showing in the settings of the admin dashboard under the `src/admin/settings` directory.

In Medusa v2, setting pages are now the same as UI routes, but they're creating under the `src/admin/routes/settings` directory.

For example, if you had a `src/admin/settings/custom/page.tsx` file in v1, you should move it to `src/admin/routes/settings/custom/page.tsx` in v2.

The file's content will be the same as a UI route. For example:

```tsx title="src/admin/routes/settings/custom/page.tsx" highlights={[["8", "9", "10", "11"]]}
import { defineRouteConfig } from "@medusajs/admin-sdk"
...
#### Setting Card Configuration Object

In v1, you exported a `config` object that showed a setting page as a card in the settings page.

Due to design changes in the Medusa Admin, setting pages are no longer shown as a card. You can only export a configuration object that adds the page to the settings sidebar.
...
In Medusa v1, admin widgets, UI routes, and settings pages received a `notify` prop to show notifications in the admin dashboard.
...
```tsx title="src/admin/widgets/product-details.tsx" highlights={[["8", "9", "10", "11"]]}
...
Medusa v2 no longer supports `medusa-react`. Instead, you can use the JS SDK with Tanstack Query to send requests in your admin customizations to the Medusa server.
...
The Medusa CLI for v2 is now in the `@medusajs/cli` package. You can refer to the [Medusa CLI reference](!resources!/medusa-cli) for the full list of commands and options.

The following table highlights the main changes between v1 and v2:
...
In v2, some of these plugins are now available as modules out-of-the-box. For example, the Stripe and SendGrid modules.

Other plugins may no longer be available, but you can still find guides to create them.
...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      <Table.Cell>
        Not available, but you can integrate it as a [Notification Module Provider](!resources!/references/notification-provider-module).
      </Table.Cell>
    </Table.Row>
    <Table.Row>
      <Table.Cell>
    ...
    <Table.Row>
      ...
      <Table.Cell>
        Not available, but you can integrate it as an [Analytics Module Provider](!resources!/references/analytics/provider).
      </Table.Cell>
    </Table.Row>
    <Table.Row>
      <Table.Cell>
...
You can find more Medusa and community integrations in the [Integrations](https://medusajs.com/integrations/) page.
...
In this section, you'll learn about the changes in specific concepts and development practices between Medusa v1 and v2.
...
- You create a module to add new features to the Medusa application.

In Medusa v2, you create a module that contains entities (called data models now) to create in the database, and services to manage those data models.
...
In Medusa v1, you added a new table to the database by creating an entity. For example, you created a `Brand` entity to add a `brand` table to the database.

In Medusa v2, you create a data model in a module to add a new table to the database. For example, you create a `Brand` data model in a Brand Module.

To create a data model, you use Medusa's Data Modeling Language (DML). It simplifies defining a table's columns, relations, and indexes with straightforward methods and configurations.
...
In Medusa v2, you can only create a service in a module. For example, you create a `BrandService` in the Brand Module at `src/modules/brand/service.ts`:
...
Learn more about in the [Service Factory](../../fundamentals/modules/service-factory/page.mdx) chapter.

#### Repositories

In Medusa v1, you used in services repositories for data models to manage them. For example, you used the `BrandRepository` to manage the `Brand` entity. Repositories were also based on TypeORM.

In Medusa v2, you generally don't need repositories for basic data-mangement features, as they're generated by the service factory. However, for more complex use cases, you can use the data model repository based on MikroORM.
...
In Medusa v1, you had access to all entities and services in the Medusa application. While this approach was flexible, it introduced complexity, was difficult to maintain, and resulted in hacky workarounds.
...
In Medusa v2, you can no longer extend services. Instead, you need to create a module with a service that contains the methods you want to add. Then, you build [workflows](../../fundamentals/workflows/page.mdx) that use both services to achieve a custom feature.

Workflows in Medusa v2 are even more flexible, allowing you to perform custom functionalities within existing workflows using concepts like [Workflow Hooks](../../fundamentals/workflows/workflow-hooks/page.mdx).
...
<Table>
  ...
  <Table.Body>
    <Table.Row>
      ...
      <Table.Cell>
        Workflows, API routes, subscribers, scheduled jobs, and custom CLI scripts.
      </Table.Cell>
    </Table.Row>
    <Table.Row>
      <Table.Cell>
...
Workflows have been optimized for data reliability, flexibilty, and more features. You can learn more in the [Workflows](../../fundamentals/workflows/page.mdx) chapters.

This section highlights the main changes in workflows between v1 and v2.

#### Workflow SDK Imports

In Medusa v1, you imported all Workflow SDK functions and types from the `@medusajs/workflow-sdk` package.

In Medusa v2, you import them from the `@medusajs/framework/workflow-sdk` package. For example:
...
- [Use when-then in workflows to run steps if a conditin is satisfied](../../fundamentals/workflows/conditions/page.mdx).
...
In Medusa v1, routes starting with `/store/me` and `/admin` where protected by default.
...
In Medusa v1, you used the `authenticate` middleware to protect API routes for admin users, `authenticateCustomer` to optionally authenticate customers, and `requireCustomerAuthentication` to require customer authentication.
...
Medusa v2 doesn't emit the same evens as v1. Refer to the [Events Reference](!resources!/reference/events) for the full list of events emitted in v2.
...
In Medusa v2, you should implement these functionalities in a [workflow](../../fundamentals/workflows/page.mdx) and call the workflow in the subscriber. By using workflows, you benefit from reliable rollback, among other features.
...
Learn more in the [Loaders](../../fundamentals/loaders/page.mdx) chapter.
...
In Medusa v2, you can't pass the `data` property in the scheduled job's configuration object.

#### Scheduled Job Implementation Changes

In Medusa v1, you implemented scheduled jobs directly in the job function.

In Medusa v2, you should implement these functionalities in a [workflow](../../fundamentals/workflows/page.mdx) and call the workflow in the scheduled job. By using workflows, you benefit from reliable rollback, among other features.
...
In Medusa v2, the cart completion process is now implemented in the [completeCartWorkflow](!resources!/references/medusa-workflows/completeCartWorkflow). There are different ways you can customize the completion process:
...
If your use case is complex and these rules are not enough. You can create a new [module](../../fundamentals/modules/page.mdx) with the necessary logic, then use that module in your custom worflows.

### Gift Card Features

Medusa v1 has gift cart features out-of-the-box.
...
Medusa also provides [Medusa Cloud](https://medusajs.com/cloud/) our managed services offering that makes deploying and operating Medusa applications possible without having to worry about configuring, scaling, and maintaining infrastructure.

Refer to the [Deployment](!resources!/deployment) documentation to learn about the deployment process for Medusa application and Next.js Starter Storefront.
